...
module Memoist
  ...
  def self.memoized_ivar_for(method_name, identifier = nil)
    "@#{memoized_prefix(identifier)}_#{escape_punctuation(OPERATOR_METHOD_NAMES.fetch(method_name, method_name))}"
  end

  def self.unmemoized_method_for(method_name, identifier = nil)
    "#{unmemoized_prefix(identifier)}_#{OPERATOR_METHOD_NAMES.fetch(method_name, method_name)}".to_sym
  end
